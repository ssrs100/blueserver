...
func ListCollections(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	log.Debug("r.RequestURI:%s", req.RequestURI)
	...
	if err := req.ParseForm(); err != nil {
		log.Error("ParseForm err:%s", err.Error())
	}
	...
	log.Debug("t1:%v, t2:%v", params["start_time"], params["end_time"])
	if t2.Before(t1) {
	...
	log.Debug("list collections:%v", collections)
	var b = Collection{}
