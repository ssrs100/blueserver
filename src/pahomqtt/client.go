...
func (c *client) Connect() Token {
	...
	go func() {
		...
		CONN:
			log.Debug("about to write new connect msg")
			c.conn, err = openConnection(broker, c.options.TLSConfig, c.options.ConnectTimeout, c.options.HTTPHeaders)
			if err == nil {
				log.Debug("socket connected to broker")
				switch c.options.ProtocolVersion {
				case 3:
					log.Debug("Using MQTT 3.1 protocol")
					cm.ProtocolName = "MQIsdp"
					cm.ProtocolVersion = 3
				case 0x83:
					log.Debug("Using MQTT 3.1b protocol")
					cm.ProtocolName = "MQIsdp"
					cm.ProtocolVersion = 0x83
				case 0x84:
					log.Debug("Using MQTT 3.1.1b protocol")
					cm.ProtocolName = "MQTT"
					cm.ProtocolVersion = 0x84
				default:
					log.Debug("Using MQTT 3.1.1 protocol")
					c.options.ProtocolVersion = 4
					cm.ProtocolName = "MQTT"
				...
				if rc != packets.Accepted {
					...
					if c.options.protocolVersionExplicit {
						log.Debug("Connecting to", broker, "CONNACK was not CONN_ACCEPTED, but rather", packets.ConnackReturnCodes[rc])
						continue
					}
					if c.options.ProtocolVersion == 4 {
						log.Debug("Trying reconnect using MQTT 3.1 protocol")
				...
				log.Debug(err.Error())
				log.Debug("failed to connect to broker, trying next")
				rc = packets.ErrNetworkError
			}
		}

		if c.conn == nil {
			log.Error("Failed to connect to a broker")
...
func (c *client) reconnect() {
		...
		for _, broker := range c.options.Servers {
			...
			if err == nil {
				...
				if rc != packets.Accepted {
					...
					//if the protocol version was explicitly set don't do any fallback
					if c.options.protocolVersionExplicit {
						log.Debug("Connecting to", broker, "CONNACK was not Accepted, but rather", packets.ConnackReturnCodes[rc])
						continue
					}
				}
				break
				...
				log.Error(err.Error())
				log.Error("failed to connect to broker, trying next")
				rc = packets.ErrNetworkError
			}
		}
		if rc != 0 {
			log.Error("Reconnect failed, sleeping for", int(sleep.Seconds()), "seconds")
			time.Sleep(sleep)
	...
	if c.connectionStatus() == disconnected {
		log.Info("Client moved to disconnected state while reconnecting, abandoning reconnect")
		return
	}
	...
	c.setConnected(connected)
	log.Debug("client is reconnected")
	if c.options.OnConnect != nil {
		go c.options.OnConnect(c)
	}
...
func (c *client) connect() (byte, bool) {
	...
	if err != nil {
		log.Error("connect got error:%s", err.Error())
	...
	if !ok {
		log.Error("received msg that was not CONNACK")
		return packets.ErrNetworkError, false
	}

	log.Debug("received connack:%v, %v", msg.ReturnCode, msg.SessionPresent)
...
func (c *client) Disconnect(quiesce uint) {
	...
	if status == connected {
		...
		log.Warn("Disconnect() called but not connected (disconnected/reconnecting)")
		c.setConnected(disconnected)
		return
	}
...
func (c *client) forceDisconnect() {
	...
	c.setConnected(disconnected)
	c.conn.Close()
	log.Debug("forcefully disconnecting")
	c.disconnect()
}
...
func (c *client) closeStop() {
	...
	select {
	case <-c.stop:
		println("In disconnect and stop channel is already closed")
	default:
		if c.stop != nil {
			close(c.stop)
		}
...
func (c *client) SubscribeMultiple(filters map[string]byte, callback MessageHandler) Token {
	var err error
	token := newToken(packets.Subscribe).(*SubscribeToken)
	log.Debug("enter SubscribeMultiple")
	if !c.IsConnected() {
		token.err = ErrNotConnected
		token.flowComplete()
		return token
	...
	c.oboundP <- &PacketAndToken{p: sub, t: token}
	log.Debug( "exit SubscribeMultiple")
	return token
}
	...
	for _, key := range storedKeys {
		...
		if isKeyOutbound(key) {
			...
			case *packets.SubscribePacket:
				if subscription {
					println(STR, fmt.Sprintf("loaded pending subscribe (%d)", details.MessageID))
					token := newToken(packets.Subscribe).(*SubscribeToken)
					c.oboundP <- &PacketAndToken{p: packet, t: token}
				}
			case *packets.UnsubscribePacket:
				if subscription {
					println(STR, fmt.Sprintf("loaded pending unsubscribe (%d)", details.MessageID))
					token := newToken(packets.Unsubscribe).(*UnsubscribeToken)
					c.oboundP <- &PacketAndToken{p: packet, t: token}
				}
			case *packets.PubrelPacket:
				println(STR, fmt.Sprintf("loaded pending pubrel (%d)", details.MessageID))
				select {
			...
			case *packets.PublishPacket:
				...
				println(STR, fmt.Sprintf("loaded pending publish (%d)", details.MessageID))
				//
				//println(STR, details)

				c.obound <- &PacketAndToken{p: packet, t: token}
			default:
				println(STR, "invalid message type in store (discarded)")
				c.persist.Del(key)
			}
		} else {
			switch packet.(type) {
			case *packets.PubrelPacket, *packets.PublishPacket:
				println(STR, fmt.Sprintf("loaded pending incomming (%d)", details.MessageID))
				select {
...
func DefaultConnectionLostHandler(client Client, reason error) {
	println("Connection lost:", reason.Error())
}
